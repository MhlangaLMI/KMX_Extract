...
Sub ExtractCoordinatesFromKMZ()
    ...
    scriptPath = xlFilePath & "\extract_coordinates.py"
    ...
    For i = LBound(coordinates) To UBound(coordinates)
        If coordinates(i) <> "" Then
            ...
            ws.Cells(row, col).Value = lat 'This is where it starts to define what goes in which column'
            ws.Cells(row, col + 1).Value = lon
            ws.Cells(row, col + 2).Value = dms_lat
            ws.Cells(row, col + 3).Value = dms_lon
            ws.Cells(row, col + 4).Value = distance
            ws.Cells(row, col + 5).Value = cumulative_distance
            ws.Cells(row, col + 6).Value = elevation
