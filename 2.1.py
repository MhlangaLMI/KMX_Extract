    ...
    with zipfile.ZipFile(kmz_file_path, 'r') as kmz:
        with kmz.open('doc.kml', 'r') as kml_file:
            ...
            for placemark in root.findall(".//kml:Placemark", namespaces=namespace):
                name = placemark.find("kml:name", namespaces=namespace).text if placemark.find("kml:name", namespaces=namespace) is not None else ""
                for coords in placemark.findall(".//kml:coordinates", namespaces=namespace):
                    ...
                    for pair in coord_pairs:
                        ...
                        coordinates_list.append((lat, lon, elevation, dms_lat, dms_lon, distance, cumulative_distance, name))
                        previous_lat = lat
                        previous_lon = lon
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("Usage: python extract_points.py <path_to_kmz_file>")
        sys.exit(1)
    ...
    try:
        ...
        for coord in coordinates:
            print(f"{coord[0]},{coord[1]},{coord[2]},{coord[3]},{coord[4]},{coord[5]},{coord[6]},{coord[7]}")
